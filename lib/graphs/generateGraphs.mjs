...
export async function drawGraphs(testProfile) {
  ...
  // Setup title
  const graphTitle = `dut:${testProfile.dutName} run:${testProfile.runName} mult:${testProfile.mult} dur:${testProfile.duration} prof:${testProfile.name}`;
  ...
  writeFileSync(`${filePath}/${fileName}.json`, JSON.stringify(data, null, 2));
}
